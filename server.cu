...
int main(int argc, char **argv)
{
    ...
    char MsgFromClient  = '0';
    float sumGPUfromclient, sum_sc;    
    ...
    //wait for the client
    read(client_sockfd, &MsgFromClient, 1);
    if (MsgFromClient == '1')
    {
        MsgFromClient++;
        write(client_sockfd, &MsgFromClient, 1);
            //close(client_sockfd);
    }
    else
    {
        perror("signal from the client is wrong");
        exit(1);
    }
    ...
    //get the sum result from the client 
    read(client_sockfd, &sumGPUfromclient, 8);
    close(client_sockfd);
    printf("  GPU Processing time: %f (ms)\n\n", GetTimer());
    sum_sc = sumGPU + sumGPUfromclient;

    printf("The result of the total is %f", sum_sc);
    // Compute on Host CPU
