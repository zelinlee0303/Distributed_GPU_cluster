...
int main(int argc, char **argv)
{
    ...
    address.sin_addr.s_addr = inet_addr("192.168.1.210");
    ...
    char MsgFromClient = '1';
    write(sockfd, &MsgFromClient, 1);
    read(sockfd, &MsgFromClient, 1);
    if (MsgFromClient == '2')
        ;
    else{
        perror("Message from the server is wrong");
        exit(1);
    }
   ...
   // StartTimer();
    ...
    //send the sum result to the server
    write(sockfd, &sumGPU, 8);
    close(sockfd);
